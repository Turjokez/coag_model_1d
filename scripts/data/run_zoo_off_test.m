% run_zoo_off_test.m
%
% Isolation test: zoo off vs best config.
% Question: is zoo grazing the main reason the deep ratio is low?
%
% Two cases:
%   1. Best config (zoo on, alpha=0.10, Da x5)
%   2. Zoo off -- everything else the same
%
...
% ---------------------------------------------------------------
cases = { ...
    struct('zoo', true,  'label', 'Zoo on (best config)', 'color', 'k'), ...
    struct('zoo', false, 'label', 'Zoo off',              'color', 'r'), ...
};
...
title('Zoo off isolation', 'FontWeight', 'normal');
saveas(gcf, fullfile(fig_dir, 'zoo_off_ratio.png'));
fprintf('\nSaved zoo_off_ratio.png\n');

% ---------------------------------------------------------------
function cfg = cfg_best(enable_zoo)
