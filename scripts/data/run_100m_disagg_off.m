% run_100m_disagg_off.m
%
% Test whether disaggregation is the dominant deep loss mechanism.
%
% From report_june12: even with r0=0 the model gives only 0.30 at 475 m.
% Microbes are not the cause. The next suspect is disaggregation removing
% the large-particle tail too fast below ~250 m.
%
% Three cases:
%   1. baseline (all physics on, r0=0.03)
%   2. disagg off  (enable_disagg = false)
%   3. disagg off + microbe off  (isolate sinking + coag only)
%
% Output: phi profile + spectrum panels at 125, 225, 375 m.
% Key question: does the large-particle tail survive to 400-500 m with
% disagg off? If yes, disaggregation is removing too much large-particle
% mass at depth.
...
% UVP at comparison depths
phi_uvp_cmp = zeros(numel(k_compare), 1);
phi_uvp_spec = zeros(numel(k_compare), n_uvp_ok);   % for spectrum
...
cases(2).label   = 'disagg OFF';
cases(2).disagg  = false;
cases(2).microbe = true;

cases(3).label   = 'disagg OFF + microbe OFF';
cases(3).disagg  = false;
...
% 4. Print ratio table
% ---------------------------------------------------------------
fprintf('\n--- Ratio table: model / UVP ---\n');
fprintf('%-10s', 'Depth(m)');
for ic = 1:n_cases
    fprintf('  %-26s', cases(ic).label);
end
fprintf('\n%s\n', repmat('-', 1, 10 + n_cases*28));
for i = 1:numel(k_compare)
    fprintf('%-10.0f', z_compare(i));
    for ic = 1:n_cases
        fprintf('  %-26.2f', ratio_table(i, ic));
    end
    fprintf('\n');
end

% ---------------------------------------------------------------
% 5. Figure 1: phi profile
% ---------------------------------------------------------------
...
hold on;
plot(phi_uvp_cmp * 1e6, z_compare, 'k--', 'LineWidth', 1.5, ...
    'DisplayName', 'UVP');
for ic = 1:n_cases
    phi_mod = sum(Y_snaps{ic}(k_compare, mask_uvp_model), 2);
...
title('Disagg off: 100 m start');
saveas(gcf, fullfile(fig_dir, 'disagg_off_profile.png'));
fprintf('Saved disagg_off_profile.png\n');

% ---------------------------------------------------------------
% 6. Figure 2: spectrum panels at 3 depths (125, 225, 375 m)
% ---------------------------------------------------------------
panel_layers = [1, 3, 6];   % indices within k_compare
n_panels     = numel(panel_layers);

dw_model_mm = diff(d_model_edges) / 1000;
...
for ip = 1:n_panels
    ...
    hold on;
    plot(d_uvp_mm, S_uvp, 'k.', 'MarkerSize', 8, 'DisplayName', 'UVP');
    for ic = 1:n_cases
        k = k_compare(ki);
        S_mod = Y_snaps{ic}(k,:) * 1e6 ./ dw_model_mm(:)';
        plot(d_model_mm, S_mod, '-', 'Color', colors(ic,:), ...
            'LineWidth', 1.5, 'DisplayName', cases(ic).label);
    end
...
fprintf('Saved disagg_off_spectrum.png\n');
