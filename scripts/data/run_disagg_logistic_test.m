% run_disagg_logistic_test.m
%
% Compare disagg_mode = 'operator_split' (current, hard D_max cutoff)
% vs 'logistic' (smooth Alldredge-style cutoff from uploaded Disaggregation.m).
%
% Both use depth-varying eps from VMP data.
% Compare size spectrum at 75 m and 200 m vs UVP on best-cast day.
...
% base config (same for both runs)
...
colors     = {'b', 'r'};
mode_names = {'hard D\_max (current)', 'logistic (Alldredge-style)'};
check_z    = [75, 200];
...
for p = 1:2
    ...
    for km = 1:2
        ...
        fprintf('  phi_sum=%.2e  max_S=%.2e  n_nonzero=%d\n', ...
            sum(phi_m), max(Sm_ok), numel(dm_ok));
        if ~isempty(dm_ok)
            loglog(dm_ok, Sm_ok, colors{km}, 'LineWidth', 1.5, ...
                'DisplayName', mode_names{km});
        end
    end
...
saveas(gcf, fullfile(fig_dir, 'disagg_logistic_test.png'));
fprintf('Saved disagg_logistic_test.png\n');
