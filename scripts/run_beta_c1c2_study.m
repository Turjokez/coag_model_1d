% run_beta_c1c2_study
% Steps:
% 1. define the power-law concentration
% 2. weight beta by C1 and C2
...
fig = plot_beta_c1c2_heatmap(diam_cm, law_names, conc_amp, conc_exp);
out_path = fullfile(fig_dir, 'beta_c1c2_heatmaps_named_laws.png');
save_figure(fig, out_path);

disp('Saved figure:');
disp(out_path);

function fig = plot_beta_c1c2_heatmap(diam_cm, law_names, conc_amp, conc_exp)
diam_um = diam_cm .* 1e4;
...
q1 = prctile(all_vals, 1);
q99 = prctile(all_vals, 99);
clim = [q1, q99];
...
cb = colorbar;
cb.Layout.Tile = 'east';
cb.Label.String = 'log10(beta ds * C1 * C2)';
title(tl, 'Differential-settling interaction (weighted)');
end
